...
| `editable_table` | **fait** — `xweb/components/editable_table.xml`. Grille éditable en ligne façon Notion : cellules texte (clic -> input, Entrée/blur sauvegarde), cellules select (étiquette colorée + menu, idiome `xweb.combobox`), cases à cocher (sauvegarde immédiate), ajout/suppression/réordonnancement de ligne (glisser-déposer, ligne déposée avant/après selon `event.clientY` vs le milieu de la ligne cible — nouveau par rapport à `xweb.kanban`, qui dépose toujours en fin de colonne), ajout de colonne (toujours type texte en v1, pas d'éditeur d'options pour "select") et en-têtes renommables (même clic-pour-éditer que les cellules). Props `id`/`save_url`/`add_row_url`/`delete_row_url`/`reorder_url`/`add_column_url`/`rename_column_url` — voir la docstring du fichier. Cinq pièges réels trouvés en l'écrivant, non triviaux — les trois premiers confirmés en cassant `scripts/verify-editable-table.mjs` (fixture statique, `scripts/render_editable_table_fixture.py`), les deux derniers avaient été trouvés **contre un vrai htmx + un vrai serveur** (`scripts/verify-editable-table-live.mjs`, retiré avec la démo `/demo/table/*` — landing_data.py — lors du recentrage sur XWeb Studio, docs/XWEB_STUDIO_CDC.md) — ni curl, ni jsdom+`_hyperscript` isolé ne les auraient montrés à l'époque :
  ...
  3. `hx-target="tbody"` (bare, sans id) cible le **premier** `<tbody>` de toute la page, pas forcément celui de cette instance : le serveur créait bien la ligne, mais le swap l'insérait dans le mauvais tableau si deux instances coexistaient. Corrigé avec un `id` par instance, même contrat que `xweb.kanban`/`modal`/`drawer`.
...
Pas d'exemple live dans ce dépôt aujourd'hui — le rendu du composant reste couvert par la fixture statique (`scripts/verify-editable-table.mjs`), pas par un vrai aller-retour serveur. |
...
| `box`, `stack`, `grid`, `container` | **fait** — `xweb/components/box.xml`/`stack.xml`/`grid.xml`/`container.xml`, `tests/test_layout_primitives.py`. Primitifs de layout génériques — xweb n'en avait aucun avant cet ajout (vérifié : zéro `t-name` container/row/col/grid/stack/box/section dans le catalogue). Nécessaires pour que le canvas du Studio (`docs/studio.md` §1 : "chaque nœud EST un appel à un composant déjà enregistré") ait de quoi arranger plusieurs composants sur une page — sans eux, seule une balise `<div>` écrite à la main aurait pu jouer ce rôle, hors du principe "canvas = arbre de t-call". `xweb.box` : `<div>` neutre, aucune classe de layout imposée. `xweb.stack` : arrangement flex (`direction`/`gap`/`wrap`/`align`/`justify`). `xweb.grid` : grille CSS à N colonnes fixes (v1 sans variante responsive par breakpoint, même choix "pas d'éditeur avancé en v1" que `xweb.rating`/`xweb.editable_table`). `xweb.container` : largeur maximale + centrage, généralisé depuis le motif déjà répété en dur dans `xweb.marketing_layout` et `templates/components_showcase.xml`. Les quatre produisent leurs classes via des dictionnaires littéraux (comme `xweb.avatar`) plutôt que par interpolation directe — chaque valeur de sortie apparaît donc déjà en texte source du fichier, aucun safelist Tailwind à ajouter (`xweb/static/xweb.css`), vérifié par un test dédié (`test_layout_primitive_classes_are_literal_source_text`). |
